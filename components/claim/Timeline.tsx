...
function TrackRow({
  ...
  return (
    <div
      className={`${ROW_GRID} items-center px-5 py-2.5 transition-colors duration-300 sm:px-7 ${
        live ? tone.surface : ""
      }`}
    >
      ...
      <motion.span
        ...
        transition={{ duration: 0.4, delay: 0.06 + index * 0.07, ease }}
        className={`inline-flex min-w-0 items-center gap-1.5 rounded-full px-3 py-1.5 text-[12.5px] font-semibold ${
          live
          ...
          {bar.hasBar ? (
            ...
            >
              <motion.span
                ...
                transition={{ duration: 0.65, delay: 0.1 + index * 0.07, ease }}
              />
              <motion.span
                initial={{ opacity: 0, x: -4 }}
                animate={{ opacity: 1, x: 0 }}
                transition={{ duration: 0.35, delay: 0.5 + index * 0.07, ease }}
                className={`tnum absolute left-full ml-2 rounded-full border px-2 py-[3px] text-[11px] font-semibold whitespace-nowrap ${
                  live
  ...
  return (
    ...
    >
      ...
      <div className="relative hidden md:block">
        ...
        {/* Date axis. */}
        <div
          className={`${ROW_GRID} border-t border-line px-5 py-2.5 sm:px-7`}
        >
    ...
    <li className="relative flex gap-3.5 pb-5 last:pb-0">
      {!isLast && (
        <>
          ...
          <motion.span
            className={`absolute top-8 bottom-0 left-[13px] w-[3px] origin-top rounded-full ${
              done ? "bg-accent-600" : tone.fill
            }`}
            initial={{ scaleY: 0 }}
            animate={{ scaleY: done ? 1 : 0 }}
            transition={{ duration: 0.45, delay: 0.14 + index * 0.1, ease }}
          />
        </>
      )}
      ...
      <motion.span
        ...
        transition={{ duration: 0.35, delay: 0.06 + index * 0.1, ease }}
        className={`relative z-10 grid h-[29px] w-[29px] shrink-0 place-items-center rounded-full ${
          done
            ? "bg-accent-600 text-white"
            : live
              ? `${tone.fill} text-white`
              : "bg-canvas text-ink-200 ring-1 ring-line"
        }`}
      >
        {done ? (
          <Check size={14} />
      ...
      <motion.div
        ...
        animate={{ opacity: 1, x: 0 }}
        transition={{ duration: 0.35, delay: 0.12 + index * 0.1, ease }}
      ...
      >
        <div className="flex flex-wrap items-center gap-x-2 gap-y-1">
          ...
          {bar.hasBar && (
            <span
              className={`tnum rounded-full px-2 py-[2px] text-[10.5px] font-semibold ${
        ...
        {bar.hasBar && (
          <div className="relative mt-2 h-1.5 w-full overflow-hidden rounded-full bg-line">
            <motion.span
              ...
              transition={{ duration: 0.6, delay: 0.18 + index * 0.1, ease }}
            />
          </div>
        )}
